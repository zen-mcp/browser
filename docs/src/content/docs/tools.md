...
| --- | --- | --- | --- |
| `browse_and_act` | Agent | Run a complex browser task from a start URL using a natural-language instruction. | `{ ok, result, instruction, start_url }` or `{ ok: false, error, error_type }`. |
...
| `get_file` | Artifacts | Read an artifact by safe file name and return base64 content. | `{ ok, file_name, mime_type, size_bytes, content_base64 }`. |
