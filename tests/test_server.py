...
class ServerTestCase(unittest.TestCase):
    ...
    def test_transport_path_maps_streamable_http_and_sse(self) -> None:
        self.assertEqual(server.transport_path("streamable-http"), "/mcp")
        self.assertEqual(server.transport_path("sse"), "/sse")

    def test_run_server_uses_transport_specific_path(self) -> None:
        settings = Settings(
            openai_api_key="test-key",
            openai_base_url="https://example.test/v1",
            model_name="test-model",
        ...
        self.assertEqual(fake_mcp.run_kwargs, {"transport": "sse", "path": "/sse"})
