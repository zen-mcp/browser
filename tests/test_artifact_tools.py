...
class FakeRuntime:
    ...
    def resolve_file_path(self, requested_name: str):
        return self.data_dir / requested_name


class ArtifactToolsTestCase(unittest.IsolatedAsyncioTestCase):
    async def test_get_file_reads_artifact(self) -> None:
        with TemporaryDirectory() as temp_dir:
            data_dir = Path(temp_dir)
            (data_dir / "artifact.txt").write_text("ok", encoding="utf-8")
            registry = ToolRegistry()
            register_artifact_tools(registry, FakeRuntime(FakePage(), data_dir))

            result = await registry.tools["get_file"]("artifact.txt")

            self.assertTrue(result["ok"])
            self.assertEqual(result["file_name"], "artifact.txt")
            self.assertEqual(result["content_base64"], "b2s=")
            self.assertEqual(
                set(result),
                {
                    "ok",
            ...
            self.assertEqual(
                set(registry.tools),
                {"list_artifacts", "get_file", "take_screenshot"},
            )
